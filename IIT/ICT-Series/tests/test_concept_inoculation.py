...
def test_play_round_returns_fraction_in_unit_interval():
    ...
    g = ConceptInoculation(
        20, seed_fraction=0.2, transmission_rate=0.5, rng=np.random.default_rng(0)
    )
    frac = g.play_round()
    assert 0.0 <= frac <= 1.0
...
def test_history_length_matches_rounds():
    ...
    g = ConceptInoculation(
        30, seed_fraction=0.1, transmission_rate=0.3, rng=np.random.default_rng(0)
    )
    g.train(100)
...
def test_transmission_converts_non_carrier():
    ...
    g = ConceptInoculation(
        30, seed_fraction=3 / 30, transmission_rate=1.0, rng=np.random.default_rng(0)
    )
...
def test_zero_transmission_no_conversion():
    ...
    g = ConceptInoculation(
        30, seed_fraction=3 / 30, transmission_rate=0.0, rng=np.random.default_rng(0)
    )
    g.train(50, burn_in=0)
    assert g.converted_beyond_seed() == 0
...
def test_exposure_increments_on_contact():
    ...
    g = ConceptInoculation(
        30, seed_fraction=3 / 30, transmission_rate=0.0, rng=np.random.default_rng(1)
    )
...
def test_instigator_decay_when_removed():
    ...
    g0 = ConceptInoculation(
        40,
        seed_fraction=0.2,
        transmission_rate=0.0,
        instigator_decay=0.0,
        forget_rate=0.0,
        rng=np.random.default_rng(0),
    )
...
def test_forget_rate_reduces_fraction_when_instigator_absent():
    ...
    common = dict(
        n_agents=40, seed_fraction=0.1, transmission_rate=0.2, instigator_decay=0.8
    )
    no_forget = []
    high_forget = []
    for s in range(4):
...
def test_carrier_fraction_method():
    ...
    g = ConceptInoculation(
        20, seed_fraction=0.25, transmission_rate=0.0, rng=np.random.default_rng(0)
    )
