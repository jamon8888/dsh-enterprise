...
def test_source_balistique_moves_away_and_stays_in_bounds():
    ...
    assert (
        np.linalg.norm(traj[-1] - traj[0]) > 1.0
        or np.linalg.norm(traj[100] - traj[0]) > 1.0
    )
...
def test_predict_source_balistique_is_accurate():
    ...
    for t in range(40, 56):
        ...
        errs.append(
            np.linalg.norm(pred - src[t + lead]) if t + lead < len(src) else 0.0
        )
...
def test_reactive_moves_toward_source():
    src = V.source_trajectory(
        "statique", 60, 32, rng=np.random.default_rng(7), start=np.array([16.0, 16.0])
    )
...
def test_capture_rate_directed_beats_random():
    ...
    src = V.source_trajectory(
        "balistique", 150, 32, rng=rng, start=np.array([16.0, 16.0])
    )
...
def test_phat_loses_or_ties_on_erratic():
    ...
    rep = V.regime_report("erratique", size=32, n_steps=200, n_seeds=4, rng=rng, lead=4)
