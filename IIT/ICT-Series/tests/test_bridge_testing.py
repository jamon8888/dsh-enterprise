...
from ict import bridge_testing as bt
...
def test_bridge_verdict_robust_across_seeds():
    ...
    verdicts = [
        bt.bridge_stability_to_recoverability(seed=s)["bridge_sigma_to_recoverability"]
        for s in (0, 1, 2)
    ]
...
def test_recovery_fraction_zero_when_delta_exceeds_width():
    ...
    small = cat.relax_to_equilibrium(
        xstar + 0.1 * np.sign(col - xstar), a, b, dt=0.01, steps=2000
    )
    assert abs(small - xstar) < 0.1
    # perturbation au-dela de la largeur -> bascule dans l'autre bassin
    big = cat.relax_to_equilibrium(
        xstar + (width + 0.5) * np.sign(col - xstar), a, b, dt=0.01, steps=2000
    )
...
def test_redundant_dataset_shape_and_structure():
    ...
    X, y = bt._redundant_feature_dataset(
        rng,
        n_samples=300,
        n_singleton=6,
        n_dup_groups=6,
...
def test_bridge3_verdict_robust_across_seeds():
    ...
    verdicts = [
        bt.bridge_extraction_to_causal_usage(seed=s)[
            "bridge_extraction_to_causal_usage"
        ]
        for s in (0, 1, 2)
    ]
...
def test_bridge3_null_control_severe_redundancy_falsifies():
    ...
    assert (
        bt.bridge_extraction_to_causal_usage(feat_noise=0.3, seed=0)[
            "bridge_extraction_to_causal_usage"
        ]
        == 1.0
    )
...
def test_bridge4_verdict_robust_across_seeds():
    """Le verdict confirme n'est pas un point-artefact (c.1014-L)."""
    verdicts = [
        bt.bridge_workspace_to_diffusion(seed=s)["bridge_workspace_to_diffusion"]
        for s in (0, 1, 2)
    ]
    assert all(v == 1.0 for v in verdicts)
...
def test_bridge5_drift_is_inert_control(verdict5):
    ...
    assert (
        abs(
            verdict5["partial_rho_compress_gen_given_drift"]
            - verdict5["rho_compress_gen"]
        )
        < 0.06
    )
...
def test_bridge5_verdict_robust_across_seeds():
    ...
    pont #1 deterministe)."""
    verdicts = [
        bt.bridge_compression_to_generalization(seed=s)[
            "bridge_compression_to_generalization"
        ]
        for s in (0, 1, 2)
    ]
    assert all(v == 1.0 for v in verdicts)
