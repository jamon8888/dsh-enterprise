...
def test_Cmu_monotone_in_n_states():
    ...
    seq_const = [0] * 200
    p_const = EM.causal_state_partition(
        seq_const, history_len=2, future_len=2, tol=1e-6
    )
    ...
    p_iid = EM.causal_state_partition(seq_iid, history_len=2, future_len=2, tol=0.05)
...
def test_excess_entropy_iid_is_zero():
    ...
    assert est["E_estimate"] < 1.0, (
        f"E iid doit etre ~ 0, recu {est['E_estimate']:.3f} (serie={est['E_series']})"
    )
...
def test_excess_entropy_markov_positive():
    ...
    assert est["E_estimate"] < 1.5, (
        f"E Markov 2 etats doit etre < log2(2) = 1 bit, recu {est['E_estimate']:.3f}"
    )
...
def test_similarity_identical_partitions_zero():
    ...
    assert sim["VI_norm"] < 0.01, (
        f"Partitions identiques doivent avoir VI_norm ~ 0, recu {sim['VI_norm']:.3f}"
    )
