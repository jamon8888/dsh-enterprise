...
class TestTriadeColocalization:
    ...
    def test_one_empty_flux_still_measures_two_pairs(self):
        ...
        r = TR.triade_colocalization(
            centers, T, n_null=200, rng=np.random.default_rng(7)
        )
    ...
    def test_keys_and_types(self):
        centers = {"beauty": [10, 20], "ignition_sae": [10, 20], "lens_agreement": [10]}
        r = TR.triade_colocalization(
            centers, 100, n_null=50, rng=np.random.default_rng(0)
        )
        ...
        assert set(r["n_per_prompt"].keys()) == {
            "beauty",
            "ignition_sae",
            "lens_agreement",
        }
...
class TestTriadeCentersWithFixtures:
    ...
    def test_returns_one_entry_per_shared_prompt(self, traces):
        ...
        for key, info in per.items():
            ...
            assert info["n_lens_agree"] <= min(info["n_ign_sae"], info["n_ign_jlens"])
