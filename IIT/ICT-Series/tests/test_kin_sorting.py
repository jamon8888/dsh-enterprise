...
from ict.self_sorting import ALGOTYPES, SelfSortingArray
...
def test_bidirectional_cures_chimeric_deadlock():
    ...
    for s in range(20):
        ...
        if SelfSortingArray(v, algotypes=algos, seed=s).run().values != sorted(v):
            minimal_failed += 1
        if KinSortingArray(
            v, algotypes=algos, seed=s, kin_affinity=False
        ).run().values != sorted(v):
            bidir_failed += 1
...
def test_no_freedom_no_aggregation():
    ...
    for s in range(5):
        ...
        assert (
            abs(m.aggregation_index(a.algotypes) - m.aggregation_index(b.algotypes))
            < 1e-9
        )
...
def test_repulsion_segregates():
    ...
    for s in range(8):
        vals, algos = _value_classes(6, 4, seed=s)
        c = KinSortingArray(
            vals, algotypes=algos, seed=s, kin_affinity=True, kin_sign=-1
        ).run()
...
def test_record_false_reaches_fixed_point_without_per_step_snapshots():
    ...
    a = KinSortingArray(vals, algotypes=algos, seed=2, kin_affinity=True).run(
        record=False
    )
    assert a.values == sorted(
        vals
    )  # tri preserve malgre l'absence de snapshots par pas
...
def test_max_steps_caps_run_before_convergence():
    ...
    for s in range(5):
        ...
        a = KinSortingArray(v, algotypes=algos, seed=s, kin_affinity=False).run(
            max_steps=5
        )
...
def test_bidirectional_false_disables_repair():
    ...
    for s in range(20):
        ...
        algos = _alternating(n)
        a = KinSortingArray(
            v, algotypes=algos, seed=s, bidirectional=False, kin_affinity=False
        ).run()
...
def test_targeting_helpers_and_frozen_exclusion():
    ...
    assert (
        KinSortingArray._kin_adjacency(["bubble", "bubble", "insertion", "insertion"])
        == 2
    )
    ...
    b = KinSortingArray(
        [1, 1, 1],
        algotypes=["insertion", "bubble", "insertion"],
