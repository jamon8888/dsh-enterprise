...
def test_recover_fraction_asym_in_unit_interval():
    ...
    xstar = prof[0][0]
    frac = ba.recover_fraction_asym_stochastic(
        xstar, 1.0, 2.0, 0.5, rng, n_trials=20, T=500
    )
    assert 0.0 <= frac <= 1.0
        ...
        for b in (1.0, 2.0, 3.5):
            for c in (-0.6, 0.0, 0.6):
                ...
                for xstar, _s, _w, _col, _bar, _d in prof:
                    fracs.append(
                        ba.recover_fraction_asym_stochastic(
...
def test_recover_fraction_asym_barrier_matters():
    ...
    frac_lo = ba.recover_fraction_asym_stochastic(
        xs_lo, 0.6, 0.5, 0.3, rng, noise=0.35, n_trials=80, T=2000
    )
    frac_hi = ba.recover_fraction_asym_stochastic(
        xs_hi, 2.5, 3.5, 0.3, rng, noise=0.35, n_trials=80, T=2000
    )
...
def test_two_covariate_partial_under_null(verdict):
    ...
    assert (
        abs(verdict["partial_rho_given_width_barrier"])
        < verdict["partial_2cov_null_p95"]
    )
...
def test_width_or_barrier_dominate_sigma(verdict):
    ...
    assert (
        verdict["rho_width_recovery"] > verdict["rho_sigma_recovery"]
...
def test_verdict_robust_across_seeds():
    ...
    verdicts = [
        ba.asym_verdict(seed=s, n_shuffle=50, n_trials=60, T=2000)["verdict"]
        for s in (0, 1, 7, 42)
    ]
