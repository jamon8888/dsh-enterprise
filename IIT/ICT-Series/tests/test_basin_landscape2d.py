...
def test_landscape2d_hessian_diagonal_eigenvalues():
    ...
    a, b, d = 1.0, 2.0, 3.0
    xstar = float(np.sqrt(b / (2.0 * a)))
    profs = bg.basin_geometry(
        bl.landscape2d_potential(a, b, d), bounds=(-2.0, 2.0, -2.0, 2.0), n_grid=30
    )
    assert len(profs) == 2
    for p in profs:
...
def test_recover_fraction_2d_in_unit_interval():
    ...
    frac = bl.recover_fraction_2d_stochastic(
        1.0, 1.0, 2.0, 3.0, rng, n_trials=20, T=500
    )
    assert 0.0 <= frac <= 1.0
        ...
        for b in (1.0, 2.0, 3.0):
            for d in (0.5, 3.0, 6.0):
                for xstar, _t in bl.landscape2d_equilibria(a, b):
                    fracs.append(
                        bl.recover_fraction_2d_stochastic(
                            float(xstar[0]),
                            a,
                            b,
                            d,
...
def test_recover_fraction_2d_sensitive_to_transverse_kick():
    ...
    f_nokick = bl.recover_fraction_2d_stochastic(
        1.0, 1.0, 2.0, 0.8, rng, n_trials=60, T=2000, transverse_kick=0.0
    )
    f_kick = bl.recover_fraction_2d_stochastic(
        1.0, 1.0, 2.0, 0.8, rng, n_trials=60, T=2000, transverse_kick=0.8
    )
...
def test_recover_fraction_2d_barrier_matters():
    ...
    rng = np.random.default_rng(42)
    prof_lo = bg.basin_geometry(
        bl.landscape2d_potential(0.6, 0.6, 2.0),
        bounds=(-2.5, 2.5, -2.5, 2.5),
        n_grid=24,
    )[0]
    prof_hi = bg.basin_geometry(
        bl.landscape2d_potential(2.5, 3.5, 2.0),
        bounds=(-2.5, 2.5, -2.5, 2.5),
        n_grid=24,
    )[0]
    assert prof_lo.barrier < prof_hi.barrier
    f_lo = bl.recover_fraction_2d_stochastic(
        float(prof_lo.xstar[0]), 0.6, 0.6, 2.0, rng, noise=0.35, n_trials=80, T=2000
    )
    f_hi = bl.recover_fraction_2d_stochastic(
        float(prof_hi.xstar[0]), 2.5, 3.5, 2.0, rng, noise=0.35, n_trials=80, T=2000
    )
    assert f_hi > f_lo
...
def test_width_or_barrier_dominate_sigma(verdict):
    ...
    assert (
        verdict["rho_width_recovery"] > verdict["rho_sigma_min_recovery"]
...
def test_verdict_robust_across_seeds():
    ...
    verdicts = [
        bl.landscape_verdict(seed=s, n_shuffle=50, n_trials=60, T=2000)["verdict"]
        for s in (0, 1, 7, 42)
    ]
