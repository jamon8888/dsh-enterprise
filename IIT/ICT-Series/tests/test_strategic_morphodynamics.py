...
def test_invasion_basin_shapes():
    ...
    A = np.array([[3.0, 0.0], [5.0, 1.0]])
    x0s, finals = M.invasion_basin(
        A, invader_idx=1, resident_idx=0, n_points=21, n_steps=300
    )
...
def test_critical_fraction_allc_resident():
    ...
    A = np.array([[3.0, 0.0], [5.0, 1.0]])
    x0s, finals = M.invasion_basin(
        A, invader_idx=1, resident_idx=0, n_points=26, n_steps=500
    )
...
def test_critical_fraction_returns_none_when_no_invasion():
    ...
    x0s, finals = M.invasion_basin(
        A, invader_idx=1, resident_idx=0, n_points=11, n_steps=200
    )
