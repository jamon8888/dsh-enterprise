...
from ict import jlens_trackP_traces as jtp
...
def _write_npz(path, meta_lens="jacobian", meta_track="P (persona)"):
    ...
    arrays = {}
    for set_name, planted, val in (
        ("persona_cautious", 7, 5.0),
...
def test_load_traces_roundtrip(trackP_npz):
    ...
    assert set(tr["prompts"]) == {
        ("persona_cautious", 0),
        ("persona_cautious", 1),
        ("persona_enthusiastic", 0),
        ("persona_enthusiastic", 1),
    }
...
def test_workspace_pipeline_runs_on_trackP_traces(trackP_npz):
    ...
    battery = workspace.event_triggered_battery(
        states,
        [e["center"] for e in events],
        window=15,
        rng=rng,
        n_shuffles=10,
        n_neutral=len(events),
    )
...
def test_real_trackP_traces_schema(variant):
    ...
    if not path.exists():
        pytest.skip(
            "traces J-Lens Track P reelles absentes (extraction GPU2 "
    ...
    assert "persona" in str(
        tr["meta"].get("persona_sets", "")
    )  # au moins un set persona
...
def test_real_trackP_control_vs_trained_separation():
    ...
    if not (p_tr.exists() and p_co.exists()):
        pytest.skip(
            "traces Track P reelles absentes (extraction GPU2 #5681 non livree)"
        )
    ...
    assert overlap <= 3, (
        f"overlap panels trained/control={overlap}/12 (attendu quasi-disjoint)"
    )
    ...
    assert sil_tr > 0.05, (
        f"trained silhouette={sil_tr:+.3f} (attendu > 0 : regroupement reel)"
    )
