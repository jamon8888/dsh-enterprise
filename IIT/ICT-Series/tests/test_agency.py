...
def test_variance_matched_noise_same_variance_different_organization():
    ...
    assert agency.structure(noise) == pytest.approx(
        agency.structure(organized), rel=1e-6
    )
...
def test_energy_gated_spectral_similarity_gates_low_energy():
    ...
    flat = np.full((32, 32), 0.5) + 1e-4 * rng.standard_normal(
        (32, 32)
    )  # quasi uniforme
...
def test_time_to_recover():
    ...
    assert (
        agency.time_to_recover(structures, target=1.0, tol=0.1, record_every=10) == 30
    )
    # cible inatteignable
