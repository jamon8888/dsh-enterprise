...
def test_invention_grows_vocabulary():
    ...
    g = InventingSignalingGame(
        4, 1, temperature=0.6, invention_rate=0.05, rng=np.random.default_rng(0)
    )
...
def test_no_invention_keeps_vocab():
    ...
    g = InventingSignalingGame(
        4, 2, temperature=0.6, invention_rate=0.0, rng=np.random.default_rng(0)
    )
...
def test_train_negative_rounds_raises():
    ...
    g = InventingSignalingGame(
        n_states=3, n_signals_init=2, rng=np.random.default_rng(0)
    )
    with pytest.raises(ValueError):
        g.train(n_rounds=-5)
...
def test_dominant_signal_per_state_empty_then_convention():
    ...
    assert len(dom) == 4
    assert all(d >= 0 for d in dom), (
        f"apres apprentissage chaque etat a un signal dominant, recu {dom}"
    )
