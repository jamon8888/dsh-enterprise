...
def test_stationary_matches_power_iteration():
    P = np.array([[0.1, 0.9, 0.0], [0.0, 0.2, 0.8], [0.3, 0.0, 0.7]])
    pi = time_arrow.stationary_distribution(P)
...
def test_entropy_production_directed_beats_reversible():
    ...
    P_rev = np.array([[0.5, 0.25, 0.25], [0.25, 0.5, 0.25], [0.25, 0.25, 0.5]])
    P_cycle = np.array(
        [
            [0.1, 0.9, 0.0, 0.0],
            [0.0, 0.1, 0.9, 0.0],
    ...
    s_rev = time_arrow.entropy_production(
        P_rev, time_arrow.stationary_distribution(P_rev)
    )
    s_cyc = time_arrow.entropy_production(
        P_cycle, time_arrow.stationary_distribution(P_cycle)
    )
    assert s_cyc > s_rev
