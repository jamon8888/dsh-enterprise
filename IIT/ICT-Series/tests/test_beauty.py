...
def test_b1_discovery_transition_produces_event():
    ...
    rep = BTY.beauty_events(
        seq, window=60, horizon=40, n_control=20, rng=np.random.default_rng(1)
    )
    ...
    assert first_step >= n_noise, (
        f"evenement attendu en phase cycle, recu step={first_step}"
    )
...
def test_b2_iid_noise_is_flat():
    ...
    rep = BTY.beauty_events(
        noise, window=60, horizon=40, n_control=20, rng=np.random.default_rng(1)
    )
...
def test_b3_pure_cycle_from_start_is_flat():
    ...
    rep = BTY.beauty_events(
        cycle, window=60, horizon=40, n_control=20, rng=np.random.default_rng(1)
    )
    assert rep["n_events"] == 0
    assert rep["verdict"] == "flat"
...
def test_beauty_events_reproducible_with_fixed_rng():
    ...
    seq = list(np.random.default_rng(0).integers(0, 4, size=400)) + [
...
def test_diagnose_couples_gate_and_fold():
    ...
    rep = BTY.diagnose(
        seq, window=60, horizon=40, n_control=10, rng=np.random.default_rng(1)
    )
...
def test_levin_complexity_formula():
    assert BTY.levin_complexity(10.0, 1024) == pytest.approx(10.0 + math.log2(1024))
