...
def test_initial_policy_is_near_uniform():
    ...
    g = SignalingGame(
        n_states=4, n_signals=4, temperature=0.5, rng=np.random.default_rng(0)
    )
...
def test_reinforcement_raises_success():
    ...
    g_learn = SignalingGame(
        n_states=4, n_signals=4, temperature=0.6, rng=np.random.default_rng(1)
    )
...
def test_convention_is_bijection_when_learned():
    ...
    g = SignalingGame(
        n_states=4, n_signals=4, temperature=0.6, rng=np.random.default_rng(0)
    )
...
def test_invalid_state_dist_raises():
    with pytest.raises(ValueError):
        SignalingGame(
            n_states=4, n_signals=4, state_dist=[0.5, 0.5]
        )  # mauvaise dimension
    with pytest.raises(ValueError):
        SignalingGame(
            n_states=4, n_signals=4, state_dist=[-0.1, 0.4, 0.4, 0.3]
        )  # negatif
...
def test_reset_restores_fresh_state():
    ...
    game = SignalingGame(
        n_states=3, n_signals=3, temperature=0.3, rng=np.random.default_rng(7)
    )
...
def test_train_anneal_restores_temperature():
    ...
    game = SignalingGame(
        n_states=3, n_signals=3, temperature=0.5, rng=np.random.default_rng(0)
    )
    t0 = game.temperature
