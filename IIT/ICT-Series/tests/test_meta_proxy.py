...
class TestObstructionVector:
    ...
    def test_antisymmetry_sign_flip(self):
        ...
        assert vec_ab["spectral_gap"] == pytest.approx(
            -vec_ba["spectral_gap"], abs=1e-9
        )
        # norm_l2 identique
    ...
    def test_scale_invariance(self):
        ...
        sig_a_scaled = {
            k: (v * 10.0 if isinstance(v, float) else v) for k, v in sig_a.items()
        }
        sig_b_scaled = {
            k: (v * 10.0 if isinstance(v, float) else v) for k, v in sig_b.items()
        }
    ...
    def test_scale_invariance_breaks_on_asymmetric_scaling(self):
        ...
        assert not math.isclose(
            vec_asym["norm_l2"], vec_baseline["norm_l2"], abs_tol=1e-6
        )
...
class TestCrossSubstratObstruction:
    ...
    def test_orthogonal_signatures_noise(self):
        ...
        subs = {
            "gray_scott": sig_a,
            "axelrod": sig_b_divergent,
            "grokking": sig_c_divergent,
        }
        out = cross_substrat_obstruction(subs)
