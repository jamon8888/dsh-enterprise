...
def test_conditioning_episode_raises_neutral_valence():
    ...
    from ict.pregnance_animat import _run_episode, _tethered_trajectories

    rng = np.random.default_rng(0)
    a = PregnanceAnimat(n_objects=3, config=AnimatConfig(), rng=rng)
    a.set_intrinsic_valences({0: 1.0})
    trajs = _tethered_trajectories(
        "balistique", 140, 32, rng, n_objects=3, neutral_idx=1, control_idx=2
    )
...
def test_hunger_grows_and_capture_satiates():
    ...
    rng = np.random.default_rng(0)
    a = PregnanceAnimat(
        n_objects=2, config=AnimatConfig(hunger_rate=0.05, satiation=0.4), rng=rng
    )
    a.set_intrinsic_valences({0: 1.0})
...
def test_build_object_trajectories_shape():
    ...
    objs = [
        ObjectSpec(idx=0, kind="balistique", intrinsic_valence=1.0),
        ObjectSpec(idx=1, kind="erratique"),
    ]
    trajs = build_object_trajectories(
        objs, n_steps=50, size=32, rng=np.random.default_rng(0)
    )
...
def test_free_energy_profile_returns_all_keys():
    ...
    for key in (
        "mse_ballistic",
        "mse_erratic",
        "mse_ratio_err_over_bal",
        "F_fixed_ballistic",
        "F_fixed_erratic",
        "F_fixed_ratio_err_over_bal",
        "F_adaptive_ballistic",
        "F_adaptive_erratic",
        "F_adaptive_ratio_err_over_bal",
        "fe_fixed_monotone_with_mse",
        "fe_adaptive_amortizes_mse",
    ):
