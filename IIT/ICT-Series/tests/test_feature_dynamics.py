...
def test_changepoint_cusum_finds_step_in_ar1_within_tolerance():
    ...
    assert abs(idx - t_true) <= 50, (
        f"CUSUM a {idx}, attendu proche de {t_true} (tol 50)"
    )
...
def test_simulate_neutral_transition_is_reproducible():
    ...
    t1, idx1 = fd.simulate_neutral_transition(
        n_tokens=200,
        transition_at=100,
        pre_mean=0.0,
        post_mean=1.0,
    ...
    t2, idx2 = fd.simulate_neutral_transition(
        n_tokens=200,
        transition_at=100,
        pre_mean=0.0,
        post_mean=1.0,
...
def test_hysteresis_residual_positive_when_drift_up():
    ...
    r = fd.hysteresis_residual(forward, backward, baseline_window=20)
    assert r > 0.4, (
        f"residu attendu > 0.4 (forward tail ~0.95, backward tail ~1.45), got {r}"
    )
