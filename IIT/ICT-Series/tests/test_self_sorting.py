...
def test_frozen_passive_is_carried():
    ...
    arr = SelfSortingArray(
        [9, 0], algotypes=["bubble", "bubble"], frozen=[False, True], seed=0
    )
    arr.run()
...
def test_robustness_passive_beats_obstacle():
    ...
    passive = SelfSortingArray(vals, frozen=fz, seed=1, frozen_mode="passive").run(
        record=False
    )
    obstacle = SelfSortingArray(vals, frozen=fz, seed=1, frozen_mode="obstacle").run(
        record=False
    )
