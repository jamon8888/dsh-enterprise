...
def test_proxy_sections_guards():
    with pytest.raises(ValueError):
        proxy_sections(
            [0, 1, 2], n_symbols=1, window_size=2, proxies={"a": lambda s, n: 0.0}
        )
    with pytest.raises(ValueError):
        proxy_sections(
            [0, 1, 2], n_symbols=3, window_size=0, proxies={"a": lambda s, n: 0.0}
        )
    with pytest.raises(ValueError):
        proxy_sections(
            [0, 1, 2],
            n_symbols=3,
            window_size=2,
            proxies={"only_one": lambda s, n: 0.0},
        )
    with pytest.raises(ValueError):
        proxy_sections(
            [0, 1],
            n_symbols=3,
            window_size=5,
            proxies={"a": lambda s, n: 0.0, "b": lambda s, n: 1.0},
        )
...
def test_cech_obstruction_test_passes():
    ...
    for seed in range(6):
        ...
        assert report["affine_verdict"] == "TRIVIAL", (
            f"seed={seed}: affine should be TRIVIAL"
        )
        assert report["multi_verdict"] == "NON_TRIVIAL", (
            f"seed={seed}: multi should be NON_TRIVIAL"
        )
...
def test_integration_real_spectral_proxies():
    ...
    proxies = {
        "spectral_gap": lambda s, n: float(
            spectral.spectral_summary(s, n)["spectral_gap"]
        ),
        "density": lambda s, n: float(spectral.spectral_summary(s, n)["density"]),
        "mean_degree": lambda s, n: float(
            spectral.spectral_summary(s, n)["mean_degree"]
        ),
    }
