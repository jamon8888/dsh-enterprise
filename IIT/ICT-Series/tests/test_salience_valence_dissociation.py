...
def test_null_reactive_inverts_motif(default_verdict):
    ...
    assert (
        abs(v["null_decision_partial_pi_given_s_reactive"]) < 0.3
    )  # pi ne predit plus
...
def test_low_power_can_fail_decision_dissociation():
    ...
    verdicts = [
        case_verdict(seed=sd, n_stimuli=40, n_trials=80)["verdict"]
        for sd in (0, 1, 7, 42)
    ]
...
def test_dissociation_full_null_inversion_at_higher_power():
    ...
    r = verdict_robust_across_seeds(
        seeds=(0, 1, 7, 42, 99), n_stimuli=160, n_trials=400
    )
