...
def test_monotonicity_is_local_not_global():
    ...
    values = [1, 3, 2, 4]
    assert m.monotonicity_error(values) == pytest.approx(
        1 / 3
    )  # une descente sur 3 paires
...
def test_delayed_gratification_counts_only_strict_rises():
    assert (
        m.delayed_gratification_events([0.9, 0.6, 0.4, 0.2]) == 0
    )  # monotone decroissant
...
def test_aggregation_index_clustered_vs_alternating():
    ...
    assert (
        m.aggregation_index(alternating) < 0.0
    )  # repulsion (mieux melange que l'aleatoire)
...
def test_aggregation_curve_pointwise():
    ...
    assert m.aggregation_curve(trajectory) == [
        m.aggregation_index(a) for a in trajectory
    ]
