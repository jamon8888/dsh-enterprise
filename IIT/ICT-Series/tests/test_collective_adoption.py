...
def test_instigators_pinned_on_target():
    ...
    g = AdoptionGame(
        10,
        3,
        instigator_fraction=0.4,
        instigator_strength=10.0,
...
def test_naive_start_uniform():
    ...
    g = AdoptionGame(
        10, 3, instigator_fraction=0.3, initial_q=1.0, rng=np.random.default_rng(0)
    )
    for a in range(g.n_agents):
...
def test_play_round_returns_payoff_in_unit_interval():
    ...
    g = AdoptionGame(
        10, 3, instigator_fraction=0.3, temperature=0.5, rng=np.random.default_rng(0)
    )
    payoff = g.play_round()
...
def test_pairing_uses_all_agents():
    ...
    g = AdoptionGame(
        8, 3, instigator_fraction=0.25, temperature=0.6, rng=np.random.default_rng(1)
    )
    g.train(400)
...
def test_training_reinforces_naive():
    ...
    g = AdoptionGame(
        10, 3, instigator_fraction=0.5, temperature=0.5, rng=np.random.default_rng(2)
    )
...
def test_no_cascade_without_instigators_control():
    ...
    report = no_cascade_without_instigators_test(
        n_agents=24, n_states=3, n_seeds=4, seed=0
    )
