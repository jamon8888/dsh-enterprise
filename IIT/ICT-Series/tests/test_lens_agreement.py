...
from ict.sae_traces import differential_features
    ...
    *,
    ...
    for si, s in enumerate(sets):
        ...
        for i in range(n_prompts):
            ...
            vals[:, F:] = 0.01 + rng.uniform(0, 1e-4, size=(T, K - F)).astype(
                np.float32
            )
            prompts[(s, i)] = {
                "ids": ids,
...
def _both_lens(bursts_a, bursts_b, **kw):
    ...
    b = _build_traces(
        bursts_b, seed=1, **kw
    )  # bruit filler distinct : lentilles differentes
    return a, b
...
def test_colocalize_lenses_same_positions_is_colocalized():
    ...
    r = la.colocalize_lenses(
        a, b, k=6, q=0.9, persistence=3, n_null=200, rng=np.random.default_rng(0)
    )
...
def test_colocalize_lenses_skips_length_mismatch():
    ...
    r = la.colocalize_lenses(
        a, b, k=6, q=0.9, persistence=3, n_null=100, rng=np.random.default_rng(0)
    )
...
def test_colocalize_lenses_reproducible_with_seed():
    ...
    assert r1["z_mean"] == r2["z_mean"] or (
        np.isnan(r1["z_mean"]) and np.isnan(r2["z_mean"])
    )
...
def test_real_fixtures_colocalization_runs():
    ...
    from ict import jlens_traces, sae_traces

    sae = sae_traces.load_traces(sae_p)
    jl = jlens_traces.load_traces(jl_p)
    r = la.colocalize_lenses(
        sae, jl, k=64, q=0.9, persistence=3, n_null=200, rng=np.random.default_rng(0)
    )
