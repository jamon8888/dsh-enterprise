...
def _panel_with_hub_and_ignitions(
    ...
    * **Ignitions (cycle tournant)** : autour de chaque centre, a chaque pas on
    ...
    valid_centers = [
        c for c in ignition_centers if ignition_half_width < c < T - ignition_half_width
    ]
...
def test_concentration_series_uniform_is_near_zero():
    ...
    assert conc.mean() < 0.05, (
        f"attendu Gini ~ 0 sur uniforme, got mean={conc.mean():.3f}"
    )
...
def test_concentration_series_concentrated_is_near_one():
    ...
    acts[:, 3] = 1.0
    conc = ws.concentration_series(acts, metric="gini")
    assert conc.mean() > 0.85, (
        f"attendu Gini ~ 1 sur concentrate, got mean={conc.mean():.3f}"
    )
...
def test_lagged_influence_diagonal_is_zero():
    ...
    acts, _ = _panel_with_hub_and_ignitions(
        seed=4, T=500, K=12, ignition_centers=(150, 300, 400)
    )
...
def test_event_triggered_battery_keys_and_structure():
    ...
    out = ws.event_triggered_battery(
        states, event_centers, window=12, rng=rng, n_shuffles=8
    )
    for key in (
        "per_event",
        "per_neutral",
        ...
        "ec_gain_neutral",
        "contrast",
        "fraction_credited_events",
        "fraction_credited_neutral",
        "n_events",
        "n_neutral",
    ):
