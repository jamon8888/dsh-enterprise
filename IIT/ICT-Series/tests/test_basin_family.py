...
def test_recover_fraction_stochastic_has_genuine_variance():
    ...
    for sigma in (2.0, 4.0, 8.0, 12.0):
        for width in (0.5, 1.0, 1.5):
            ...
            xstar = prof[1][0]
            fracs.append(
                bf.recover_fraction_stochastic(
...
def test_recover_fraction_stochastic_barrier_matters():
    ...
    frac_lo = bf.recover_fraction_stochastic(
        prof_lo[0], a_lo, b_lo, rng, noise=0.35, n_trials=80, T=2000
    )
    frac_hi = bf.recover_fraction_stochastic(
        prof_hi[0], a_hi, b_hi, rng, noise=0.35, n_trials=80, T=2000
    )
...
def test_verdict_robust_across_seeds():
    ...
    verdicts = [
        bf.pont1bis_verdict(seed=s, n_shuffle=50)["verdict"] for s in (0, 1, 7, 42)
    ]
