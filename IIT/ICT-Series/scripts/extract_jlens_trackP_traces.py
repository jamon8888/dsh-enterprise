...
DEFAULT_LENS_FILENAME = (
    "qwen3.5-4b/jlens/Salesforce-wikitext/Qwen3.5-4B_jacobian_lens.pt"
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="défaut = lens 4B-base vérifié (n=1000). Alternative "
        "qualité-max : '.../Qwen3.5-4B_jacobian_lens_n1000.pt'.",
    )
    p.add_argument(
        "--k",
        type=int,
...
def main() -> None:
    ...
    hf = transformers.AutoModelForCausalLM.from_pretrained(args.model, **cfg_kwargs).to(
        device
    )
    hf.eval()
    ...
    print(
        f"[model] classe={type(hf).__name__} n_layers={n_layers} "
    ...
    if is_instruct:
        print(
            "[persona] modèle instruct détecté : persona double (prompt + post-training)."
        )
    else:
        print(
            "[persona] base model : persona elicité PAR le prompt (paradigme workspace-"
    ...
    print("[jlens] HF model wrappé via jlens.from_hf.")
    ...
    if args.stage == "smoke":
        ...
        sets = {
            "persona_cautious": PERSONA_PROMPT_SETS["persona_cautious"][:1],
    ...
    if args.stage == "full":
        ...
        meta = {
            ...
            "mono_token_vocab_restricted": (
                "J-lens = mono-token, vocabulaire-restreint "
                "(limitation déclarée Anthropic)"
            ),
            "persona_methodology": (
                "Persona elicité par conditionnement du prompt "
            ...
            "semantic_divergence_vs_sae": (
                "SAE top-k : hors top-k = 0 EXACT. "
            ...
            "cpu_drafted_untested": (
                "Drafted CPU po-2025 (torch+cpu) 2026-07-11, "
