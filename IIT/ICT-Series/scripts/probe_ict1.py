...
print(
    f"  min={min(vals):.4f} max={max(vals):.4f} moyenne={sum(vals) / len(vals):.4f} amplitude={max(vals) - min(vals):.4f}"
)
...
print(
    f"  min={min(vx):.4f} max={max(vx):.4f} moyenne={sum(vx) / len(vx):.4f} amplitude={max(vx) - min(vx):.4f}"
)
...
for start in [(0, 0, 0), (1, 0, 0), (0, 1, 0), (1, 1, 1)]:
    ...
    print(
        f"     Phi le long = {[round(p, 3) for p in phis]}  events={T.detect_events(phis)}"
    )
...
for bit in range(3):
    ...
    print(
        f"  flip noeud {bit}: {base_state}->{pert}  Phi={[round(p, 3) for p in phis]}  retour_meme_attracteur={same}"
    )
