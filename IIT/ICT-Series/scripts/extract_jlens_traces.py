...
def probe_lens_fit_n(lens_repo: str, lens_filename: str) -> int | None:
    ...
    try:
        ...
        print(
            f"[lens] convergence.csv non lu (best-effort) : {type(exc).__name__}: {exc}"
        )
        return None
...
def main() -> None:
    ...
    if "instruct" in args.model.lower():
        sys.exit(
            "ERREUR: Track S #5681 = Qwen3.5-9B-BASE (pas instruct). "
    ...
    if args.stage == "full":
        ...
        meta = {
            ...
            "semantic_divergence_vs_sae": (
                "SAE top-k : hors top-k = 0 EXACT (densify exact). "
            ...
            "cpu_drafted_untested": (
                "Drafted CPU po-2025 (torch+cpu), validé+exécuté GPU2 "
