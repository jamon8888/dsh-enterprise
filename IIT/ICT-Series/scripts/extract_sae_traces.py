...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out-dir", default=str(Path(__file__).resolve().parent.parent / "traces")
    )
...
def guard_single_gpu() -> torch.device:
    ...
    if n != 1:
        sys.exit(
            f"ERREUR: {n} GPUs visibles — lancer avec CUDA_VISIBLE_DEVICES=2 "
...
def find_decoder_layers(
    model: torch.nn.Module, n_layers_hint: int | None
) -> torch.nn.ModuleList:
    ...
    print(
        f"[model] couches decodeur : '{name}' ({n} couches, "
...
def load_sae(sae_repo: str, layer: int, device: torch.device):
    ...
    print(
        f"[sae] {sae_repo} layer{layer} — cles : "
...
def apply_control_permutation(model: torch.nn.Module, seed: int) -> None:
    ...
    print(
        f"[control] input embeddings permutes ({emb.weight.shape[0]} lignes, seed={seed})"
    )
...
def main() -> None:
    ...
    if args.k is None:
        ...
        if not args.allow_k_override:
            ...
            print(
                f"[k] OVERRIDE explicite {k} != officiel {k_official} : "
    ...
    if args.stage == "full":
        ...
        out_path = out_dir / trace_filename(
            ...
            n_layers=n_layers,
            n_clamp=len(clamp_ids),
        )
        if out_path.exists() and not args.overwrite:
            sys.exit(
                f"ERREUR: {out_path} existe deja. Deux runs d'echelles "
    ...
    model.eval()
    print(
        f"[model] classe={type(model).__name__} "
    ...
    if args.stage == "smoke":
        sets = {
            "code_python": PROMPT_SETS["code_python"][:1],
        ...
        for i, text in enumerate(prompts):
            ...
            if n_bad:
                ...
                if n_bad > 0.05 * vals.shape[0]:
                    sys.exit(
                        f"ERREUR: {set_name}__{i} : {n_bad}/{vals.shape[0]} "
                ...
                print(
                    f"[warn] {set_name}__{i} : {n_bad} position(s) top-k "
            ...
            print(
                f"[trace] {key}: T={hidden.shape[0]} L0 moy={l0.mean():.1f} "
    ...
    if args.stage == "smoke" and args.variant == "trained" and not clamp_ids:
        ...
        probe = tokenizer("La capitale de la France est", return_tensors="pt").to(
            device
        )
        with torch.no_grad():
            gen = model.generate(**probe, max_new_tokens=12, do_sample=False)
        print(
            f"[sanity] greedy: {tokenizer.decode(gen[0], skip_special_tokens=True)!r}"
        )
    ...
    print(
        f"[done] stage={args.stage} variant={args.variant} en {time.time() - t0:.0f}s"
    )
