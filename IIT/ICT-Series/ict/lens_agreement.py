...
def lens_ignition_centers(
    ...
    *,
    ...
    for key, dense in panels.items():
        ...
        centers = [
            int(ev["center"])
            for ev in ignition_events(conc, thr, persistence=persistence)
        ]
        out[key] = (centers, T)
    return out
...
def colocalize_lenses(
    ...
    **saute** (positions non comparables) et compte dans ``n_skipped`` -- garde-fou
    ...
    ev_a = lens_ignition_centers(
        traces_a, feat_a, q=q, persistence=persistence, metric=metric
    )
    ev_b = lens_ignition_centers(
        traces_b, feat_b, q=q, persistence=persistence, metric=metric
    )
