...
def acts_topk_panels(
    traces: dict, feature_ids: np.ndarray
) -> dict[tuple[str, int], np.ndarray]:
    """Panels denses [T, K] float32 par prompt pour ``feature_ids`` (schema amende)."""
    return {
        key: densify(e["ids"], e["vals"], feature_ids)
...
def states_from_panel(bits: np.ndarray) -> np.ndarray:
    ...
    if F > 20:
        raise ValueError(
            f"panel a {F} features : bit-packing limite a 20 "
...
# --------------------------------------------------------------------------- #
def resolve_capture_layer(
    n_layers: int, layer: int | None = None, layer_frac: float | None = None
) -> dict:
    ...
    if (layer is None) == (layer_frac is None):
        raise ValueError(
            "fournir exactement un de layer / layer_frac "
            f"(recu layer={layer}, layer_frac={layer_frac})"
        )
    if n_layers is None or n_layers < 1:
        raise ValueError(
            f"n_layers invalide : {n_layers!r} — la config du "
...
def check_sae_model_match(
    sae_d_model: int, model_d_model: int, sae_repo: str = "", model: str = ""
) -> None:
...
def assert_bf16_readout(
    quantization_config: object | None, allow_quantized: bool = False
) -> None:
