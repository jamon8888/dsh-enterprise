...
from .self_sorting import ALGOTYPES, Cell, Probe
...
class KinSortingArray:
    ...
    def __init__(
        ...
        if not (len(algotypes) == len(frozen) == n):
            raise ValueError(
                "values, algotypes et frozen doivent avoir la meme longueur"
            )
    ...
    # ----------------------------------------------------------- mouvements de tri
    def _sort_target(self, i: int) -> int | None:
    ...
    def _kin_adjacency(algotypes: list) -> int:
        ...
        return sum(
            1 for k in range(len(algotypes) - 1) if algotypes[k] == algotypes[k + 1]
        )

    def _kin_target(self, i: int) -> int | None:
        ...
        for j in (i - 1, i + 1):
            if (
                0 <= j < len(self.cells)
                and not self.cells[j].frozen
                and self.cells[j].value == c.value
            ):
                trial = algos[:]
