...
def recover_fraction_2d_stochastic(
    xstar: float,
    a: float,
    b: float,
    d: float,
...
def _profile_quantities(prof: BasinProfile) -> tuple[float, float, float, float, float]:
    ...
    * ``anisotropy`` = lambda_max / lambda_min (>= 1).
    ...
    return (
        ...
        float(prof.width),
        float(prof.barrier),
        float(prof.anisotropy),
    )


def _gather_landscape_family(
    a_grid: np.ndarray,
    b_grid: np.ndarray,
    d_grid: np.ndarray,
    noise: float,
    ...
    for a in a_grid:
        for b in b_grid:
            for dd in d_grid:
                profs = basin_geometry(
                    landscape2d_potential(float(a), float(b), float(dd)),
...
def landscape_verdict(
    ...
    * ``INCONCLUSIVE`` : decouplage insuffisant ou signal sous le bruit.
    ...
    sig_m, sig_min, wid, bar, aniso, rec = (
        fam["sigma_mean"],
        fam["sigma_min"],
        fam["width"],
        fam["barrier"],
...
def landscape_recoupled_null(
    ...
    for b in b_grid:
        profs = basin_geometry(
            landscape2d_potential(float(a_fixed), float(b), float(d_fixed)),
            bounds=(-2.5, 2.5, -2.5, 2.5),
            n_grid=24,
        )
        if len(profs) != 2:
