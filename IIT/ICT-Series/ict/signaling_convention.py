...
def _softmax(
    propensities: np.ndarray, temperature: float, rng: np.random.Generator
) -> int:
...
class SignalingGame:
    ...
    def __init__(
        ...
        *,
        ...
        state_dist: Sequence[float] | None = None,
        initial_q: float = 1.0,
        ...
        if state_dist is None:
            ...
            if state_dist.sum() <= 0.0 or np.any(state_dist < 0.0):
                raise ValueError(
                    "state_dist doit etre une distribution (>= 0, somme > 0)."
                )
    ...
    def reset(self) -> None:
        ...
        self.Q_r = np.full(
            (self.n_signals, self.n_actions), self.initial_q, dtype=float
        )
    ...
    def train(self, n_rounds: int, anneal_to: float | None = None) -> None:
...
def _trajectory(
    game: SignalingGame, n_rounds: int, *, anneal_to: float | None, n_probes: int = 40
    ...
    for c in checkpoints:
        block = c - prev
        if block > 0:
            game.joint_state_signal[:] = (
                0.0  # mesurer la politique courante sur ce bloc
            )
...
def coordination_emerges_test(
    ...
    # Apprenti.
    g = SignalingGame(
        n_states, n_signals, temperature=temperature, rng=np.random.default_rng(seed)
    )
...
def vocabulary_bottleneck_test(
    ...
    def _train_mi(n_sig: int) -> float:
        g = SignalingGame(
            n_states, n_sig, temperature=temperature, rng=np.random.default_rng(seed)
        )
        g.train(n_rounds, anneal_to=anneal_to)
        g.joint_state_signal[:] = 0.0
        for _ in range(window):
...
def mi_tracks_meaning_test(
    ...
    g = SignalingGame(
        n_states, n_states, temperature=temperature, rng=np.random.default_rng(seed)
    )
