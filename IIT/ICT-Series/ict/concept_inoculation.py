    ...
    def __init__(
        ...
        *,
        ...
        if not 0.0 <= transmission_rate <= 1.0:
            raise ValueError(
                f"transmission_rate in [0, 1] requis (recu {transmission_rate})."
            )
        if not 0.0 <= confirmation_bias <= 1.0:
            raise ValueError(
                f"confirmation_bias in [0, 1] requis (recu {confirmation_bias})."
            )
        if not 0.0 <= instigator_decay <= 1.0:
            raise ValueError(
                f"instigator_decay in [0, 1] requis (recu {instigator_decay})."
            )
    ...
    def play_round(self, *, instigator_present: bool = True) -> float:
        ...
        for i in range(0, self.n_agents - 1, 2):
            ...
            if oa != ob:  # un porteur, un non-porteur
                ...
                eff_rate = self.transmission_rate * (1.0 - self.confirmation_bias) ** (
                    self.exposure[target] - 1
                )
                if self.rng.random() < eff_rate and self.opinion[target] == 0:
                    ...
                    reinfected[target] = (
                        True  # un converti expose a un porteur = reinfecte
                    )
            elif oa == 1 and ob == 1:
        ...
        self.history.append(
            {
                ...
                "new_since_seed": max(
                    0, n_carriers - n_carriers_before + self._instigators_active()
                ),
...
def confirmation_bias_throttles_test(
    ...
    for bias in biases:
        ...
        for s in range(n_seeds):
            g = ConceptInoculation(
                n_agents,
                seed_fraction=4 / n_agents,
                transmission_rate=0.2,
                confirmation_bias=bias,
                rng=np.random.default_rng(seed + 100 * int(bias * 10) + s),
            )
            g.train(n_rounds, burn_in=0)
    ...
    throttles = (
        1.0 if (high > 0.9 and low < 0.6 and monotone_decrease and has_drop) else 0.0
    )
    return {
        "biases": biases,
