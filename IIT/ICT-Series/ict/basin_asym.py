...
def relax_asym(
    x0: float, a: float, b: float, c: float, dt: float = 0.01, steps: int = 5000
) -> float:
...
def recover_fraction_asym_stochastic(
    xstar: float,
    a: float,
    b: float,
    c: float,
...
def _stratify_decouple(
    fam: dict[str, np.ndarray], n_bins: int = 5
) -> dict[str, np.ndarray]:
...
def asym_verdict(
    ...
    * ``INCONCLUSIVE`` : ``partial_2cov < -0.2`` ou decouplage insuffisant.
    ...
    fam = _gather_asym_family(
        a_grid, b_grid, c_grid, noise, n_trials, T, dt, perturb_frac, seed
    )
...
def asym_recoupled_null(
    ...
    recoveries: list[float] = []
    for b in b_grid:
        for xstar, sigma, width, col, barrier, depth in asym_basin_profile(
            a_fixed, float(b), float(c_fixed)
        ):
            frac = recover_fraction_asym_stochastic(
                xstar,
                a_fixed,
