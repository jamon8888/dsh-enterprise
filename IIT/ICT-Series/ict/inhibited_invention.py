...
class InhibitedInventingGame(InventingSignalingGame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if inhibition_growth < 0.0:
            raise ValueError(
                f"inhibition_growth >= 0 requis (recu {inhibition_growth})."
            )
    ...
    def play_round(self, reinforce: bool = True):
        ...
        if coordinated == 0:
            self.inhibition_level = min(
                1.0, self.inhibition_level + self.inhibition_growth
            )
        else:
            self.inhibition_level = max(
                0.0, self.inhibition_level - self.inhibition_decay
            )
    ...
    def mean_inhibition(self, window: int = 0) -> float:
        ...
        if not self.inhibition_history:
            return 0.0
        recent = (
            self.inhibition_history[-window:] if window > 0 else self.inhibition_history
        )
    ...
    for g in growths:
        ...
        for s in range(n_seeds):
            game = InhibitedInventingGame(
                ...
                invention_cost=0.0,
                inhibition_growth=g,
                rng=np.random.default_rng(seed + 100 * len(vocab_means) + s),
            )
...
def learned_helplessness_test(
    ...
    for s in range(n_seeds):
        game = InhibitedInventingGame(
            ...
            temperature=0.6,
            invention_rate=0.1,
            inhibition_growth=0.08,
            rng=np.random.default_rng(seed + s),
        )
        game.train(n_rounds, anneal_to=0.15)
        finals.append(game.final_inhibition())
        mid = (
            game.inhibition_history[n_rounds // 2]
            if len(game.inhibition_history) > n_rounds // 2
            else 0.0
        )
        mids.append(mid)
        vocabs.append(game.final_vocab_size())
    ...
    helplessness = (
        ...
        if (
            final_inh > 0.8
            and final_inh >= mid_inh - 0.1
            and mean_vocab < n_states
            and mean_coord < 0.9
        )
        else 0.0
    )
...
def trap_persists_under_more_compute_test(
    ...
    persistent_trap = (
        1.0
        if (
            res["inhibited_coord_2n"] < 0.5
            and res["free_coord_2n"] > 0.8
            and inhibited_gain < 0.1
        )
        else 0.0
    )
