...
def branching_avalanche_size(
    mu: float, rng: random.Random, max_size: int = 100000
) -> int:
...
# --------------------------------------------------------------------------- #
def hill_alpha(data: list, xmin: float, discrete: bool = False) -> float | None:
...
def ks_distance(data: list, xmin: float, alpha: float) -> float | None:
...
def exponential_rate(data: list, xmin: float) -> float | None:
...
def select_xmin(
    data: list, candidates: list | None = None, discrete: bool = False
) -> float | None:
...
def compare_powerlaw_exponential(
    ...
    if rng is not None and gof_boot > 0:
        out["gof_pvalue"] = gof_pvalue(
            data, xmin, alpha, rng, n_boot=gof_boot, discrete=discrete
        )
    # 1. garde-fou d'adequation (KS) -- rejette le faux scale-free a l'oeil
    if out["ks"] is not None and out["ks"] > ks_max:
        out["verdict"] = (
            "echelle caracteristique (ajustement loi de puissance insuffisant)"
        )
        return out
