...
def local_compressibility(
    ...
    K = np.array(
        [
            compressed_length(seq[t - window : t], level=level)
...
def _matched_iid_control_max(
    ...
    for _ in range(int(n_control)):
        ...
        _, d = compressibility_gain_curve(
            iid, window=window, horizon=horizon, smooth=smooth, level=level
        )
...
def fold_in_compressibility_curve(
    ...
    **Exploratoire** : ce diagnostic ne prouve pas une vraie fronce (il
    ...
    d1_scale = (
        np.median(np.abs(d1[np.isfinite(d1)])) if np.any(np.isfinite(d1)) else 0.0
    )
...
def diagnose(
    ...
    **kw,
    ...
    """
    rep = beauty_events(
        states, window=window, horizon=horizon, smooth=smooth, level=level, **kw
    )
...
def levin_complexity(program_bits: float, runtime_steps: float) -> float:
    ...
    if runtime_steps < 1:
        raise ValueError(f"runtime_steps doit etre >= 1, recu {runtime_steps}")
...
def powerplay_frontier(theories: Sequence) -> dict:
    ...
    **toutes** les taches deja resolues par la theorie courante **et** au moins
    ...
    return {"frontier": accepted, "rejected": rejected, "total_accepted": len(accepted)}


def mdl_frontier(
    states: Sequence,
    prefix_schedule: Sequence[int] | None = None,
    split: float = 0.5,
    n_points: int = 10,
) -> dict:
    ...
    **Caveat honnete (cf comments #7258)** : appliquee a la *string de
    ...
    if prefix_schedule is None:
        ...
        if n <= lo:
            ...
            grid = {
                int(round(lo + (n - lo) * i / max(n_points - 1, 1)))
...
def mdl_compression_progress(
    ...
    """
    front = mdl_frontier(
        states, prefix_schedule=prefix_schedule, split=split, n_points=n_points
    )
    rows = front["rows"]
    h = int(horizon)
    if h < 1:
