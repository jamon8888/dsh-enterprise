...
def empirical_model(
    ...
    alphabet = (
        tuple(int(o) for o in outcomes) if outcomes is not None else tuple(observed)
    )
    if len(alphabet) < 2:
        raise ValueError(
    ...
    if unknown:
        raise ValueError(
            f"issues hors alphabet {alphabet} dans les supports : {sorted(unknown)}"
        )
    ...
    *quels protocoles de mesure* portent le conflit.
    ...
    while changed:
        ...
        for ci in list(keep):
            ...
            sub = EmpiricalModel(
                contexts=tuple(model.contexts[i] for i in trial),
...
def contextuality_verdict(
    ...
    *,
    ...
    satisfiable = (
        sat_exhaustive if sat_exhaustive is not None else cp.get("satisfiable")
    )
    ...
    if not satisfiable:
        ...
        if not too_large:
            out["minimal_obstruction"] = minimal_obstruction(
                model, max_search=max_search
            )
        return out
...
def model_from_signatures(
    ...
    *,
    ...
    thr = (
        ...
        else median_split_thresholds(signatures, proxies)
    )
