...
def triade_centers(
    ...
    * ``lens_agreement`` -- intersection ensembliste des centres SAE et
    ...
    ev_sae = LA.lens_ignition_centers(
        traces_sae, feat_sae, q=q, persistence=persistence, metric=metric
    )
    ev_jlens = LA.lens_ignition_centers(
        traces_jlens, feat_jlens, q=q, persistence=persistence, metric=metric
    )
    ...
    for key in shared:
        ...
        if len(tokens) != T:
            ...
            tokens = (
                tokens[:T] if len(tokens) > T else (tokens + [""] * (T - len(tokens)))
            )
        ...
        rep = BTY.beauty_events(
            seq, window=window, horizon=horizon, n_control=n_control, rng=rng
        )
...
def _panels_for_key(
    traces: dict, key: tuple[str, int], feature_ids: np.ndarray
) -> list[dict]:
...
def triade_colocalization(
    ...
    * ``"undefined"`` si **2 flux ou plus** sont vides (pas de triade a
    ...
    series = {
        f: [int(x) for x in (centers.get(f) or []) if 0 <= int(x) < T] for f in fluxes
    }
    ...
    * sinon -> ``"chance"`` (regle du minoritaire).
    ...
    if not defined or all(v == 0 for v in defined.values()):
        ...
        if defined.get("colocalized", 0) > 0 and defined.get("colocalized", 0) >= max(
            defined.get("partial", 0),
            defined.get("chance", 0),
