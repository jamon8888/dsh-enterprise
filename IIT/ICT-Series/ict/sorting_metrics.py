...
def distance_to_target(values: list, target: list | None = None) -> int:
...
def aggregation_index(algotypes: list) -> float:
    ...
    if n < 2:
        return 0.0
    observed = sum(1 for i in range(n - 1) if algotypes[i] == algotypes[i + 1]) / (
        n - 1
    )
...
def recovery_time(curve: list, perturbation_step: int, tol: float = 1e-9) -> int | None:
