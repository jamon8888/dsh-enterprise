...
def _make_slowing_variance(
    n: int, sigma_base: float, drift: float, seed: int = 0
) -> np.ndarray:
...
def test_ews_report_dataclass_all_expected_fields():
    ...
    report = fd.ews_report(
        np.random.default_rng(0).normal(0, 1, 200), window=30, thin_factor=1
    )
    expected_fields = {
        "variance",
        "ar1",
