...
def test_kappa_c_predicted_constant_value():
    ...
    assert np.isclose(psr.KAPPA_C_PREDICTED, expected), (
        f"KAPPA_C_PREDICTED = {psr.KAPPA_C_PREDICTED}, attendu = {expected}"
    )
...
def test_estimate_stability_boundary_returns_per_seed_values():
    ...
    assert set(boundary.keys()) == expected_keys, (
        f"cles boundary attendues {expected_keys}, recues {set(boundary.keys())}"
    )

    kcps = boundary["kappa_critical_per_seed"]
    assert kcps.shape == (5,)
    assert (kcps > 0).all(), f"kappa_critical_per_seed doit etre > 0, recu {kcps}"
...
def test_delieur_verdict_is_stable():
    ...
    assert set(delieur.keys()) == expected_keys, (
        f"cles delieur attendues {expected_keys}, recues {set(delieur.keys())}"
    )
    ...
    assert delieur["delieur_ratio_max"] < psr.RATIO_DIVERGENT, (
        f"delieur_ratio_max = {delieur['delieur_ratio_max']} >= {psr.RATIO_DIVERGENT}"
    )
...
def test_predict_and_dissociate_returns_valid_verdict():
    ...
    assert set(result.keys()) == expected_keys, (
        f"cles verdict attendues {expected_keys}, recues {set(result.keys())}"
    )
...
def test_run_full_protocol_composes_scan_and_verdict():
    ...
    s = result["scan"]
    assert (
        v["boundary"]["kappa_critical_median"]
        == psr.estimate_stability_boundary(s)["kappa_critical_median"]
    )
...
def test_run_full_protocol_is_deterministic():
    ...
    assert np.array_equal(r1["scan"]["divergent_mask"], r2["scan"]["divergent_mask"])
