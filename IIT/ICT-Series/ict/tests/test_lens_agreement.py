...
from ict import lens_agreement as la
...
from ict import sae_traces as _sae_traces
...
def _patch_panels(
    monkeypatch, dense_a: np.ndarray, dense_b: np.ndarray, prompts=("p0",)
):
    ...
    def fake_acts_topk_panels(traces, feature_ids):
    ...
    def fake_differential_features(traces, k=64):
    ...
    monkeypatch.setattr(
        _sae_traces, "differential_features", fake_differential_features
    )
...
def test_colocalize_lenses_identical_lens_is_colocalized(monkeypatch):
    ...
    assert r["z_mean"] > 5.0, (
        f"attendu z >> 0 pour lentilles identiques, recu {r['z_mean']}"
    )
...
def test_colocalize_lenses_skips_prompts_with_incompatible_T(monkeypatch):
    ...
    def fake_acts_topk_panels_a(traces, feature_ids):
        # Cote A : T=400 pour les deux prompts.
        return {("p_short", 0): dense_a_short, ("p_long", 0): dense_a_long}

    def fake_acts_topk_panels_b(traces, feature_ids):
    ...
    def lens_ignition_centers_factory(side_panels):
        ...
        def fake_lens_ignition_centers(
            traces, feature_ids, *, q=0.9, persistence=3, metric="gini"
        ):
    ...
    monkeypatch.setattr(
        la, "lens_ignition_centers", lens_ignition_centers_factory(side_panels_a)
    )
    ...
    def fake_lens_ignition_centers_seq(traces, feature_ids, **kw):
        call_count["n"] += 1
        if call_count["n"] == 1:
            return lens_ignition_centers_factory(side_panels_a)(
                traces, feature_ids, **kw
            )
        else:
            return lens_ignition_centers_factory(side_panels_b)(
                traces, feature_ids, **kw
            )
...
def test_colocalize_lenses_synthetic_verdict_aggregation(monkeypatch):
    ...
    def fake_event_coloc(centers_a, centers_b, T, n_null=500, rng=None):
...
def test_colocalize_lenses_dominant_dissociated_verdict(monkeypatch):
    ...
    monkeypatch.setattr(la, "event_colocalization", fake_event_coloc)
    r = la.colocalize_lenses(
        traces, traces, k=K_FIXTURE, n_null=10, rng=np.random.default_rng(0)
    )
