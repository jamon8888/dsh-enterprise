...
def test_sigmoid_numerically_stable_and_symmetric():
    ...
    out = svd._sigmoid(x)
    assert np.all((out >= 0.0) & (out <= 1.0)), f"_sigmoid hors [0,1] : {out}"
...
def test_approach_probability_valence_shape_range_gain_zero_half():
    ...
    assert np.allclose(p0, 0.5), f"gain=0 doit donner 0.5 partout, recu {p0}"
...
def test_verdict_robust_across_seeds_structure_and_robustness():
    ...
    assert set(out.keys()) == expected_keys, (
        f"cles verdict_robust attendues {expected_keys}, recues {set(out.keys())}"
    )
