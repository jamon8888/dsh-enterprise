...
def tpm_description_length(counts: np.ndarray) -> float:
    ...
    if arr.ndim != 2 or arr.shape[0] != arr.shape[1]:
        raise ValueError(f"counts doit etre une matrice carree, recu shape {arr.shape}")
    if np.any(arr < 0):
...
# ---------------------------------------------------------- code deux parties
def two_part_code(
    states: Sequence, split: float = 0.5, states_to_index: dict | None = None
) -> dict:
...
def entropy_rate_estimate(states: Sequence, block: int = 2) -> dict:
    ...
    if not grams:
        return {"H_block": 0.0, "entropy_rate": 0.0, "vocab_size": 0, "n_blocks": 0}
    ...
    **maximum** a entropie intermediaire entre 0 (determinisme) et la
    ...
    for t_id, traj in enumerate(trajectories):
        ...
        try:
            ...
            kg = {"k_gain": float("nan"), "len_real": 0, "len_shuffled": float("nan")}
        for block in blocks:
            if n < max(block, 2):
                continue
            try:
