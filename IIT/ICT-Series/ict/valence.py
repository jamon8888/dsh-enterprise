...
def valence_at(
    ...
    if obstacles is not None and len(obstacles):
        for o in np.asarray(obstacles, dtype=float):
            v -= _OBST_AMP * float(
                np.exp(-float(np.sum((p - o) ** 2)) / (2.0 * _OBST_SIGMA**2))
            )
    return v
...
def valence_gradient(
    ...
    g = (
        _BUMP_AMP
        * float(np.exp(-float(np.sum((p - s) ** 2)) / (2.0 * sigma**2)))
        * ((s - p) / (sigma**2))
    )
    if obstacles is not None and len(obstacles):
        for o in np.asarray(obstacles, dtype=float):
...
def _obstacle_repulsion(pos: np.ndarray, obstacles: np.ndarray | None) -> np.ndarray:
...
def role_switching(traj: np.ndarray, min_turn: float = 0.5) -> float:
    ...
    norms = (
        np.linalg.norm(moves[1:], axis=1) * np.linalg.norm(moves[:-1], axis=1) + 1e-12
    )
    cos = dots / norms
...
def regime_report(
    ...
    for s in range(n_seeds):
        src = source_trajectory(
            kind, n_steps, size, rng=rng, speed=speed, start=center_start(size)
        )
        tr_r = simulate_reactive(src, size, start, n_steps, obstacles=obstacles)
        tr_p = simulate_phat(
            src, size, start, n_steps, lead=lead, alpha=alpha, obstacles=obstacles
        )
        tr_a = random_walk(size, start, n_steps, rng=rng)
        for name, tr in (("reactif", tr_r), ("phat", tr_p), ("aleatoire", tr_a)):
            acc[name]["capture"].append(
                capture_rate(tr, src, radius=capture_radius, burn=capture_burn)
            )
...
def moving_average_trajectory_2d(
    source_traj: np.ndarray, window: int = 5
) -> np.ndarray:
