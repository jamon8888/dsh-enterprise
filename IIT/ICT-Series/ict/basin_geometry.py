...
def numeric_hessian(V: Potential, x: np.ndarray, h: float = 1e-4) -> np.ndarray:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            xmm[i] -= h
            xmm[j] -= h
            val = (float(V(xpp)) - float(V(xpm)) - float(V(xmp)) + float(V(xmm))) / (
                4.0 * h * h
            )
            H[i, j] = val
            H[j, i] = val
...
def _bissect_zero(
    V: Potential, a: float, b: float, tol: float = 1e-10, max_iter: int = 100
) -> float:
...
def _newton_root_2d(
    ...
    for _ in range(max_iter):
        ...
        if box is not None and not (
            box[0] <= x[0] <= box[1] and box[2] <= x[1] <= box[3]
        ):
            return None
    return None
...
def basin_geometry(
    ...
    raise ValueError(
        f"bounds doit etre 2-tuple (1D) ou 4-tuple (2D), recut {len(bounds)}"
    )


def _basin_geometry_1d(
    V: Potential, x_min: float, x_max: float, n_grid: int
) -> list[BasinProfile]:
...
def _basin_geometry_2d(
    V: Potential, x_min: float, x_max: float, y_min: float, y_max: float, n_grid: int
) -> list[BasinProfile]:
...
def profile_cusp(a: float, b: float, n_grid: int = 400) -> list[BasinProfile]:
    ...
    """
    return basin_geometry(
        lambda x: float(x[0]) ** 4 / 4.0 + a * float(x[0]) ** 2 / 2.0 + b * float(x[0]),
        bounds=(-3.0, 3.0),
        n_grid=n_grid,
    )
...
def profile_double_well(a: float, b: float, n_grid: int = 400) -> list[BasinProfile]:
    ...
    """
    return basin_geometry(
        lambda x: a * float(x[0]) ** 4 - b * float(x[0]) ** 2,
        bounds=(-3.0, 3.0),
        n_grid=n_grid,
    )
