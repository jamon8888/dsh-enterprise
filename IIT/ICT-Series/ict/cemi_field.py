...
def coherence_energy_ratio(
    field_f: DipoleField | None = None,
    *,
    n_side: int = 32,
    f_sync: float = 1.0,
    power: float = 1.0,
    ...
    """
    async_field = DipoleField(
        n_side=n_side, f_sync=0.0, power=power, rng=np.random.default_rng(seed)
    )
    f_field = field_f or DipoleField(
        n_side=n_side, f_sync=f_sync, power=power, rng=np.random.default_rng(seed)
    )
...
def sync_window_sweep(
    n_side: int = 32,
    power: float = 1.0,
    seeds: tuple[int, ...] = (0, 1, 2, 3, 4),
    f_values: tuple[float, ...] | None = None,
) -> list[dict]:
    ...
    for f in f_values:
        ratios: list[float] = []
        for seed in seeds:
            res = coherence_energy_ratio(
                n_side=n_side, f_sync=f, power=power, seed=seed
            )
...
def negative_control_power(
    n_side: int = 32, power_gain: float = 2.0, seed: int = 0
) -> dict:
    ...
    coherent/incoherent.
    """
    base = coherence_energy_ratio(n_side=n_side, f_sync=0.0, power=1.0, seed=seed)
    ...
    boosted_field = DipoleField(
        n_side=n_side, f_sync=0.0, power=power_gain, rng=np.random.default_rng(seed)
    )
