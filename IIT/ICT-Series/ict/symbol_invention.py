...
class InventingSignalingGame:
    ...
    def __init__(
        ...
        *,
        ...
        if not 0.0 <= invention_rate <= 1.0:
            raise ValueError(
                f"invention_rate dans [0, 1] requis (recu {invention_rate})."
            )
    ...
    def play_round(self, reinforce: bool = True) -> tuple[int, int, int, float]:
        ...
        if reinforce and coordinated == 0 and self.n_signals < self.max_signals:
            ...
            if (
                recent_fail_rate > self.invention_cost
                and self.rng.random() < self.invention_rate
            ):
...
def invention_cost_tradeoff_test(
    ...
    strictly_decreasing = any(
        vocab_per_cost[i] > vocab_per_cost[i + 1]
        for i in range(len(vocab_per_cost) - 1)
    )
    cost_threshold = (
        vocab_free >= n_states and vocab_costly < n_states and strictly_decreasing
    )
    return {
        "n_states": float(n_states),
