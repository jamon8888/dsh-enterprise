...
def _recover_fraction(
    ...
    n_back = 0
    for d in delta_grid:
        x = cat.relax_to_equilibrium(
            xstar + direction * float(d), a, b, dt=dt, steps=full_steps
        )
        if abs(x - xstar) < eps:
            n_back += 1
...
def _partial_spearman(sig: np.ndarray, rec: np.ndarray, wid: np.ndarray) -> float:
...
def bridge_stability_to_recoverability(
    ...
    **largeur de bassin** (le col est-il franchi ?), pas de raideur locale. La
    ...
    for a in a_grid:
        for b in b_grid:
            for xstar, sigma, width, col in basin_geometry(float(a), float(b)):
                frac = _recover_fraction(
                    xstar, col, float(a), float(b), delta_grid, dt, full_steps, eps
                )
                sigmas.append(sigma)
                widths.append(width)
    ...
    null_partial = np.array(
        [
            _partial_spearman(rng.permutation(sig), rec, wid)
    ...
    **correlation partielle** (importance | unicite) -> ablation, calculee **par
    ...
    for _ in range(int(n_datasets)):
        ...
        null_partials = np.array(
            [
                _partial_spearman(rng.permutation(marg), abl, uniq)
                for _ in range(int(n_shuffle))
            ]
        )
...
def bridge_workspace_to_diffusion(
    ...
    *affine* le signal, c.1024-L : concurrent reel, pas orthogonal).
    ...
    null_partial = np.array(
        [
            _partial_spearman(rng.permutation(cap), frac, n_un)
            for _ in range(int(n_shuffle))
        ]
    )
    p95_partial_null = float(np.percentile(np.abs(null_partial), 95))
    ...
    bridge = (
        1.0
        if (partial > p95_partial_null and partial > 0.2 and null_control_frac < 0.05)
        else 0.0
    )
...
def _markov_sequence(
    rng: np.random.Generator, n: int, n_states: int, regularity: float, drift: float
) -> list[int]:
...
def _markov_sequence_two_regime(
    rng: np.random.Generator,
    n_half: int,
    n_states: int,
    reg_train: float,
...
def bridge_compression_to_generalization(
    ...
    * **regime non-stationnaire** (:func:`_markov_sequence_two_regime`, train
    ...
    for _ in range(int(n_trials)):
        ...
        gen_s.append(g)
        drift_s.append(dr)
    comp_s = np.asarray(comp_s)
    gen_s = np.asarray(gen_s)
    drift_s = np.asarray(drift_s)
    ...
    for _ in range(int(n_trials)):
        ...
        seq = _markov_sequence_two_regime(
            rng,
            n // 2,
            n_states,
            reg_train,
            0.0,  # train compressible
            0.0,
            0.5,
        )  # test decale
        c, g = _score(seq)
        comp_ns.append(c)
        gen_ns.append(g)
    comp_ns = np.asarray(comp_ns)
    gen_ns = np.asarray(gen_ns)
    rho_compress_gen_nonstationary = _spearman(comp_ns, gen_ns)

    bridge = (
        1.0
        if (rho_compress_gen > 0.7 and rho_compress_gen_nonstationary < -0.3)
        else 0.0
    )
...
def bridge_recoverability_to_agency(
    ...
    **meme score final** ``recovery_score`` peut etre atteint par deux systemes
    ...
    for trial in range(int(n_trials)):
        ...
        struct_true = [float(np.var(s[mask])) for s in snaps]
