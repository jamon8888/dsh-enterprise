...
def concentration_series(acts: np.ndarray, metric: str = "gini") -> np.ndarray:
    ...
    if metric == "gini":
        ...
        raise ValueError(
            f"metric inconnu: {metric!r} (attendu 'gini' ou 'participation_ratio')"
        )
    return out
    ...
    **Reutilise ``synthesis.emergence_gain`` par import, pas par copie** : la
    ...
    if neutral is None:
        ...
        if available and target_n > 0:
            idx = rng.choice(
                len(available), size=min(target_n, len(available)), replace=True
            )
...
def event_colocalization(
    ...
    **phase** relative a A — c'est le controle standard du co-firing de trains de
    ...
    null = np.array(
        [_sym_nn(a, [(y + int(s)) % T for y in b]) for s in shifts], dtype=float
    )
