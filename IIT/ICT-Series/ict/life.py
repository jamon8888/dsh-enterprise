...
def canonical_pattern(name: str) -> np.ndarray:
    ...
    rows = _PATTERNS[name]
    return np.array(
        [[1 if ch == "O" else 0 for ch in row] for row in rows], dtype=np.uint8
    )
...
def _bounding_box(grid: np.ndarray) -> tuple[np.ndarray, tuple[int, int]] | None:
...
def period_and_displacement(
    grid: np.ndarray, max_steps: int = 64
...
def calibrate(name: str, size: int = 32) -> tuple[bool, dict[str, object]]:
    ...
    period, displacement = period_and_displacement(
        embed(canonical_pattern(name), size), max_steps=2 * size
    )
...
def random_soup(size: int, density: float, rng: np.random.Generator) -> np.ndarray:
    ...
    if not 0.0 < density < 1.0:
        raise ValueError(
            f"random_soup : density dans (0, 1) requis (density={density})"
        )
    ...
    for i in range(H):
        for j in range(W):
            if g[i, j] and not seen[i, j]:
                ...
                crop = np.zeros(
                    (max(ys) - min(ys) + 1, max(xs) - min(xs) + 1), dtype=np.uint8
                )
                for y, x in cells:
