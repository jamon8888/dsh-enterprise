...
from . import time_arrow, tpm_estimation
...
class DungAF:
    ...
    def induced(self, subset: Iterable[int]) -> DungAF:
        ...
        sub_attacks = [(a, b) for (a, b) in self.attacks if a in keep and b in keep]
...
def _phase_a_report() -> str:
    ...
    lines.append("=" * 72)
    lines.append(
        "Source reelle : AF canoniques de la serie Tweety "
        "(SymbolicAI/Tweety/Tweety-5-Abstract-Argumentation.ipynb)."
    )
    lines.append("")
    verdict_pass = True
    cases = [
        ...
        ("Chaine lineaire 0->1->2->3->4", controversial_chain(5), [0, 1, 2, 3, 4]),
    ]
    for name, af, order in cases:
        ...
        lines.append(
            f"    TPM {P.shape[0]}x{P.shape[1]}, {n_states} etats, "
            f"sigma={diag['sigma']:.4f} nats, "
            f"P_rev_dist={diag['P_rev_distance']:.4f}"
        )
        lines.append(f"    Phase A [{'PASS' if ok else 'FAIL'}] (>= 2 etats requis)")
        lines.append("")
    lines.append("-" * 72)
