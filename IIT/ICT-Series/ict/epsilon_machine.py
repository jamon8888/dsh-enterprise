...
def _future_distribution(
    states: Sequence, t: int, history_len: int, future_len: int
) -> np.ndarray:
...
def _merge_history_future(
    histories: list[tuple], states: Sequence, history_len: int, future_len: int
...
def causal_state_partition(
    ...
    while changed:
        ...
        centroids = np.array(
            [np.mean([dists[i] for i in grp], axis=0) for grp in signatures]
        )
        m = len(signatures)
...
def partition_similarity(
    ...
    if total == 0:
        return {"VI_norm": 0.0, "VI_raw": 0.0, "n_used": 0, "agree": 0, "disagree": 0}
