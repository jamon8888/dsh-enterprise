    ...
    def __init__(
        ...
        *,
        ...
        if instigator_strength <= 0.0:
            raise ValueError(
                f"instigator_strength > 0 requis (recu {instigator_strength})."
            )
        if temperature <= 0.0:
...
def critical_threshold_test(
    ...
    *,
    ...
    rho_c = (
        float((rhos_arr[jump_idx] + rhos_arr[jump_idx + 1]) / 2.0)
        if len(rhos_arr) > 1
        else 0.0
    )
