...
class NonStationaryBandit:
    ...
    **changement de bras optimal** à mi-horizon : la structure de
    ...
    def __init__(
        ...
        probs_pre: tuple[float, ...] | None = None,
        probs_post: tuple[float, ...] | None = None,
        seed: int = 0,
    ) -> None:
        self.n_arms = n_arms
...
def recovery_rate(
    actions: np.ndarray, env: NonStationaryBandit, window: int | None = None
) -> float:
    ...
    correct = sum(1 for i, t in enumerate(range(s, end)) if post[i] == opt(t))
