...
def transport_residual(
    ...
    if s_i.shape != s_j.shape:
        raise ValueError(f"sections de formes differentes : {s_i.shape} vs {s_j.shape}")
    if s_i.size < 2:
        raise ValueError(">= 2 fenêtres requis pour un ajustement affine.")
...
def cech_obstruction_class(
    ...
    *,
        ...
        for a in range(n):
            for b in range(a + 1, n):
                for c in range(b + 1, n):
                    ...
                    key_ca = (
                        (names[a], names[c])
                        if (names[a], names[c]) in residuals
                        else (names[c], names[a])
                    )
    ...
    return {
        "n_proxies": n,
        "n_windows": len(next(iter(secs.values()))) if secs else 0,
