...
def do_kick(
    ...
    if lo is not None or hi is not None:
        kicked = np.clip(
            kicked, lo if lo is not None else -np.inf, hi if hi is not None else np.inf
        )
    return kicked
...
def stake_index(
    ...
    steps: int,
    anchor: float,
    free_step_fn: StepFn | None = None,
    free_init: np.ndarray | None = None,
) -> float:
...
def restoring_step(
    state: np.ndarray, strength: float = 0.3, rng: np.random.Generator | None = None
) -> np.ndarray:
...
def drift_step(
    state: np.ndarray, bias: float = 0.4, rng: np.random.Generator | None = None
) -> np.ndarray:
