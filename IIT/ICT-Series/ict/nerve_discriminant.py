...
def nerve_b1(
    ...
    finite_mask = (
        np.isfinite(h1_intervals).all(axis=1)
        if h1_intervals.size
        else np.array([], dtype=bool)
    )
    h1_finite = h1_intervals[finite_mask] if h1_intervals.size else np.zeros((0, 2))
    persistences = (
        (h1_finite[:, 1] - h1_finite[:, 0]) if h1_finite.size else np.array([])
    )
    ...
    if eps <= 0:
        ...
        edges = [
            s for s, filt in st_inst.get_filtration() if len(s) == 2 and filt <= eps
        ]
        triangles = [
            s for s, filt in st_inst.get_filtration() if len(s) == 3 and filt <= eps
        ]
        n_e = len(edges)
        n_t = len(triangles)
