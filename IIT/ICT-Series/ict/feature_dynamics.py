...
def ews_report(
    ...
    """
    summary = ew.ews_summary(
        trace, window=window, thin_factor=thin_factor, detrend_sigma=detrend_sigma
    )
...
def simulate_neutral_transition(
    ...
    *trop* (faux positifs). Si le CUSUM detecte la transition, c'est attendu.
    ...
    for t in range(1, transition_at):
        trace[t] = (
            pre_mean + pre_ar1 * (trace[t - 1] - pre_mean) + sigma * g.standard_normal()
        )
    # regime post
    for t in range(transition_at, n_tokens):
        trace[t] = (
            post_mean
            + post_ar1 * (trace[t - 1] - post_mean)
            + sigma * g.standard_normal()
        )
