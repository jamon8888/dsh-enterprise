...
def agency_measures_at_scale(
    ...
    rec_RD_final = A.recovery_score(
        V_ref_b, V_abl_b, block_average(snaps[-1], block), mask_b
    )
    ...
    structures_b = [
        float(np.var(block_average(s, block)[mask_b]))
        if mask_b.any()
...
# --------------------------------------------------------------------------- #
def pearson_corr(x: list[float], y: list[float]) -> tuple[float | None, int]:
    ...
    r = float(
        np.dot(xm, ym) / (np.sqrt(np.dot(xm, xm)) * np.sqrt(np.dot(ym, ym)) + 1e-12)
    )
