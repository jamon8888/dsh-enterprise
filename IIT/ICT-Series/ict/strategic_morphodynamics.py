...
def make_strategies(rng: np.random.Generator | None = None) -> dict[str, Strategy]:
...
def grim_continuation_payoff(
    ...
    g_grim = _grim_discounted(
        delta, n_rounds, opponent="grim", invader="grim", T=T, R=R, P=P, S=S
    )
    g_alld = _grim_discounted(
        delta, n_rounds, opponent="grim", invader="alld", T=T, R=R, P=P, S=S
    )
    return g_grim, g_alld


def _grim_discounted(
    delta: float,
    max_rounds: int,
    opponent: str = "grim",
    invader: str = "grim",
...
def grim_resists_threshold(
    ...
    crossing = (
        float(delta_grid[np.argmax(diffs >= 0)]) if np.any(diffs >= 0) else float("nan")
    )
...
def critical_fraction(
    x0s: np.ndarray, finals: np.ndarray, tol: float = 0.5
) -> float | None:
...
def noise_collapse(
    ...
    for k, noise in enumerate(noise_grid):
        scores = round_robin(
            strategies, n_rounds=n_rounds, noise=float(noise), n_reps=n_reps, rng=rng
        )
        for n in strategies:
            out[n][k] = scores[n]
    return out
