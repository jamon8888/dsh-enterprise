...
def time_reversal(P: np.ndarray, pi: np.ndarray) -> np.ndarray:
    ...
    if P.shape[0] != P.shape[1] or P.shape[0] != pi.shape[0]:
        raise ValueError(f"formes incoherentes : P={P.shape}, pi={pi.shape}")
...
def compare_real_reversed_reversibilized(
    ...
    return {
        ...
        "dist_real_vs_reversibilized": 0.5
        * float(np.sum(np.abs(P_real - P_reversibilized))),
        "dist_reversed_vs_reversibilized": 0.5
        * float(np.sum(np.abs(P_reversed - P_reversibilized))),
        "n_symbols": int(n_symbols),
        "mapping": dict(mapping),
    }
