...
def build_object_trajectories(
    ...
    start_offsets: dict[int, np.ndarray] | None = None,
) -> np.ndarray:
...
class PregnanceAnimat:
    ...
    def reset(self, start: np.ndarray | None = None) -> None:
        self.pos = (
    ...
    def _feasibility(self, phat_future: np.ndarray) -> np.ndarray:
        ...
        cur = np.array(
            [
                self._hist[i][-1] if self._hist[i] else np.zeros(2)
                for i in range(self.n_objects)
            ]
        )
        excess = np.linalg.norm(phat_future - cur, axis=1) - float(
            self.cfg.lead * self.cfg.step
        )
    ...
    def _aim_point(
        ...
        if (
            pi_target > 0.2
            and target < phat_future.shape[0]
    ...
    def approach_fraction(
        self, obj_idx: int, obj_traj: np.ndarray, window: tuple[int, int] | None = None
    ) -> float:
...
def prediction_accuracy_test(
    ...
    err_phat = anticipation_error_2d(
        phat_predicted_trajectory(src, lead, alpha), src, lead
    )
...
def _tethered_trajectories(
    ...
    if (
        control_idx is not None
        and control_idx < n_objects
        and control_idx != neutral_idx
    ):
...
def embodied_transfer_test(
    ...
    cond_trajs = _tethered_trajectories(
        kind,
        n_condition,
        size,
        rng,
    ...
    neutral_traj = source_trajectory(
        kind, n_test, size, rng=rng, speed=0.3, start=np.array([4.0, 4.0])
    )
    cond_approach = _mean_approach(
        n_objects, cfg, animat.lv.pi, neutral_traj, neutral_idx, base_seed=seed + 7
    )

    # --- controle : signal non-conditionne (pi nul) SEUL, meme protocole ---
    zero_pi = np.zeros(n_objects)
    control_approach = _mean_approach(
        n_objects, cfg, zero_pi, neutral_traj, neutral_idx, base_seed=seed + 7
    )
...
def action_commitment_test(
    ...
    test_full = np.full((n_objects, n_test, 2), 1e4)
    test_full[neutral_idx] = neutral_traj
    a_inv = PregnanceAnimat(
        n_objects=n_objects, config=cfg, rng=np.random.default_rng(seed + 3)
    )
...
def behavioral_reversibility_test(
    ...
    def _approach_when(pi_vector):
        return _mean_approach(
            n_objects, cfg, pi_vector, neutral_traj, neutral_idx, base_seed=seed + 5
        )
    ...
    ext_neutral = source_trajectory(
        kind, n_extinction, size, rng=rng, speed=0.3, start=np.array([4.0, 4.0])
    )
    ...
    reversible = (
        acquired_approach > extinguished_approach + 0.15
        and acquired_pi > 0.3
        and extinguished_pi < acquired_pi * 0.5
    )
    return {
...
def free_energy_profile_test(
    ...
    def _profile(k: str) -> dict[str, float]:
        ...
        for i in (0, 1):
            ...
            f_fix.append(
                float(np.mean(fe.gaussian_surprise(o_future, p_past, sigma=1.0)))
            )
...
def predictive_information_regime_contrast(
    ...
    mi_ratio = err["mutual_information_bits"] / max(
        bal["mutual_information_bits"], 1e-12
    )
    mse_ratio = err["mse"] / max(bal["mse"], 1e-12)
    mi_anticorrelated = 1.0 if (mi_ratio < 1.0 and mse_ratio > 1.0) else 0.0
    return {
...
def dissociation_matrix(
    ...
    dissociation_observed = (
        erratic["err_phat_vs_persistence"]
        > measures["balistique"]["err_phat_vs_persistence"]
        and erratic["transferred"] == 1.0
        and erratic["reversible"] == 1.0
    )
    measures["_verdict"] = {
        "dissociation_observed": 1.0 if dissociation_observed else 0.0
    }
    return measures
