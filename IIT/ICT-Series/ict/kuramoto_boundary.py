...
def region_masks() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    band[BAND_ROWS[0] : BAND_ROWS[1], :] = True
...
def disc_mask(center: tuple[float, float], radius: float) -> np.ndarray:
    r2 = (np.arange(N)[:, None] - center[0]) ** 2 + (
        np.arange(N)[None, :] - center[1]
    ) ** 2
...
def make_frequency_profile(rng: np.random.Generator) -> np.ndarray:
    ...
    omega[BAND_ROWS[0] : BAND_ROWS[1], :] = rng.normal(
        0.0, 0.3, (BAND_ROWS[1] - BAND_ROWS[0], N)
    )
...
def bootstrap_ic95(values: list[float]) -> tuple[float, float]:
    ...
    arr = np.array(values)
    medians = [
        float(np.median(arr[rng.integers(0, len(arr), len(arr))]))
...
def run_protocol() -> dict[str, Any]:
    ...
    for seed in SEEDS:
        ...
        twin_detail.append(
            {
                "seed": seed,
                "twin_1": run_1["r_cross"],
                "twin_2": run_2["r_cross"],
                "diff": d,
            }
        )
    ...
    if rejected >= 2:
        ...
        verdict = (
            "FALSIFIED (topologie inerte : delta_topo < eps_topo sur >= 3 graines)"
        )
    elif (
        med_topo >= 0.25
        and med_topo <= 0.75
    ...
    borderline = bool(
        ...
        and (
            abs(med_topo - 0.25) <= 0.05
            or abs(med_topo - 0.75) <= 0.05
            or abs(med_delta - 0.10) <= 0.05
        )
    )
    ...
    if not (math.isnan(med_post) or math.isnan(med_topo)):
        if med_post >= med_topo + 0.10:
            ...
            disruption_verdict = (
                "EPIPHENOMENE (delta < eps_topo — lecture causale retiree)"
            )
        else:
    ...
    return {
        "protocol": {
            ...
            "band_rows": list(BAND_ROWS),
            "seeds": list(SEEDS),
            "pairs": len(PAIR_GEOMETRY),
            "numpy_only_cpu": True,
        },
        "calibration_null": {
...
def main() -> None:
    ...
    print(
        "=== Case 6 — frontiere topologique vs frontiere fonctionnelle (Kuramoto 2D) ==="
    )
    print(
        f"Calibration null (avant test) : sigma_delta = {cal['sigma_delta']:.4f}, eps_topo = {cal['eps_topo']:.4f}"
    )
