...
def recover_fraction_stochastic(
    xstar: float,
    a: float,
    b: float,
    ...
    * **largeur** : geometrie (marge avant le col).
    ...
    for _ in range(int(n_trials)):
        ...
        for _ in range(int(T)):
            x = (
                x
                + float(double_well_force(x, a, b)) * float(dt)
                + sdt * rng.standard_normal()
            )
...
def partial_spearman(
    x: np.ndarray, y: np.ndarray, covariates: Sequence[np.ndarray]
) -> float:
...
def pont1bis_verdict(
    ...
    * ``INCONCLUSIVE`` : ``partial < -0.2`` (effet inverse marque) ou signal
    ...
    fam = _gather_family(
        sigma_grid, width_grid, noise, n_trials, T, dt, perturb_frac, seed
    )
