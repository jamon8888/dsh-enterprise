...
| `dsh-enterprise/packages/enterprise/kb-rag/src/plugin.ts:25` | `ponytail: in-memory substring search, pgvector cosine when PG + pgvector lands` | `watchtower` PG `pgvector` extension lands (`knowledge_search` cosine) |
...
| 8 | `kb-rag` | ✅ scaffolded `packages/enterprise/kb-rag` (4 tests) — `ctx.effect('kb-rag')` + `knowledge_search` delegates to `ctx.get('kb-rag').search` | `watchtower` PG `pgvector` for cosine |
| 9 | `dsh-library` | ✅ scaffolded `packages/enterprise/dsh-library` (4 tests) — `ctx.effect('dsh-library')` + `cite` | `fs.read` `.dsh/library` when populated |
| 10 | `dsh-mneme` | ✅ scaffolded `packages/enterprise/dsh-mneme` (3 tests) — `Map` store/recall + `ctx.on('session/event')` auto-store, `better-sqlite3` optional | `watchtower` PG `run_events` or SQLite file |
